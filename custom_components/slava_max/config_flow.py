...
PERMISSION_OPTIONS = [
    ...
    {
        "value": "vacuum",
        "label": "Робот-пылесос Масяня",
    },
    {"value": "braga", "label": "Braga Controller"},
    {
        "value": "braga_emergency",
        "label": "Braga: аварийная остановка",
    },
    {"value": "status", "label": "Состояние дома"},
    {"value": "intercom", "label": "Домофон"},
    {
        "value": "intercom_open",
        "label": "Домофон: открытие двери",
    },
...
def _normalize_users(
    ...
    for raw_id, raw_profile in value.items():
        ...
        result[user_id] = {
            ...
            CONF_USER_ENABLED: bool(
                profile.get(CONF_USER_ENABLED, True)
            ),
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            try:
                ...
                data[CONF_TARGET_ID] = int(
                    data[CONF_TARGET_ID]
                )

                return self.async_create_entry(
                    title=f"Slava MAX — {title}",
                    data=data,
                )
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Required(
                        CONF_POLLING, default=True
                    ): bool,
                    vol.Optional(
                        CONF_ALLOWED_USERS, default=""
                    ): str,
                }
            ),
            errors=errors,
...
class SlavaMaxOptionsFlow(OptionsFlowWithReload):
    ...
    def _users(
        ...
        return _normalize_users(
            self._current().get(CONF_USERS, {})
        )
    ...
    async def async_step_users(
        ...
        return self.async_show_menu(
            ...
            description_placeholders={
                ...
                "users": ", ".join(
                    (
                        f"{profile.get(CONF_USER_NAME) or user_id}"
                        f" ({user_id})"
                    )
    ...
    async def async_step_add_detected_user_details(
        ...
        if (
            not self._selected_user_id
            or not self._detected_user
        ):
